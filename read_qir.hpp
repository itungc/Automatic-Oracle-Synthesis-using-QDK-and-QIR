...
namespace detail
{
    ...
    template <typename Ntk> struct read_qir_impl
    {
        ...
        Ntk run()
        {
            ...
            {
                if (arg.getType()->isIntegerTy(1u)) // Boolean
                {
                    value_signals[&arg] = {ntk.create_pi()};
                }
                else if (arg.getType()->isIntegerTy(64u)) // 64-bit integer
                {
                    value_signals[&arg].resize(64u); // Change here for different bit-width
                    std::generate(
            ...
            {
                fmt::print("[e] function signature not supported: inputs must be Bool or Integers and return type must be Bool, Integer, or\n"
                           "Bool tuple\n");
                std::abort();
            }
      ...
      private:
        ...
        typename std::vector<typename Ntk::signal> const& get_signal(Ntk& ntk, llvm::Value const* value)
        {
            ...
            {
                value_signals[value] = constant_word(ntk, constant->getValue().getZExtValue(), 64u); // Change here for different bit-width
            ...
            for (auto const& inst : block)
            {
                ...
                case llvm::Instruction::ICmp:
                {
                    ...
                    case llvm::ICmpInst::Predicate::ICMP_SGT:
                    {
                        ...
                        auto copy = get_signal(ntk, inst.getOperand(0u));
                        mockturtle::carry_ripple_subtractor_inplace(
                            ntk, copy, get_signal(ntk, inst.getOperand(1u)), carry);
                ...
                case llvm::Instruction::Mul:
                {
                    ...
                    if (ty0->isIntegerTy(64) && ty1->isIntegerTy(64))
                    {
                        ...
                        modular_multiplication_inplace(ntk, value_signals[&inst], value_signals[inst.getOperand(1u)], size); 
                    }
                    else
                    {
                ...
                case llvm::Instruction::SRem:
                {
                    ...
                    if (prevInst) {
                        ...
                        if (prevOpcode == llvm::Instruction::Add)
                        {
                            ...
                            // Assuming op2 is the inst.getOperand(1u) value
                ...
                case llvm::Instruction::Add:
                {
                    ...
                    // Different cases and conditions are not needed.
                    if (ty0->isIntegerTy(64) && ty1->isIntegerTy(64))
                    {
                        fmt::print("[i] Entered the integer condition \n");
                        auto carry = ntk.get_constant(false); // carry set as false
                        carry_ripple_adder_inplace(ntk, value_signals[&inst], value_signals[inst.getOperand(1u)], carry);
                        // modular_adder_inplace(ntk, value_signals[&inst], value_signals[inst.getOperand(1u)]);

                    }
                    else
                    {
                        fmt::print("[i] Entered the Boolean condition \n");
                        modular_adder_inplace(ntk, value_signals[&inst], value_signals[inst.getOperand(1u)]);
                    }
                }
                break;
        ...
        bool analyze_function_signature(llvm::Function const& f) const
        {
            ...
            {
                // For integer input, the "arg.getType() -> isIntegerTy(1u)" is false
                // std::cout << arg.getType() -> isIntegerTy(1u) << "\n";
                // std::cout << typeid(arg).name() << "\n";abc
                // 1u is the bitwidth of the integer, I change it to 64. Now it can take integer as input

                //Check if argument is integer (64) or boolean (1u)
