...
namespace Microsoft.Quantum.OracleGenerator {
    ...
    operation CompareLessThan(inputs : (Qubit[], Qubit[]), output : Qubit) : Unit {
        // The implementation of this operation will be
        // automatically derived from the description in
        // OracleGenerator.Classical.CompareLessThan
    }

    @EntryPoint()
    operation RunProgram() : Unit {
        InitOracleGeneratorFor(Microsoft.Quantum.OracleGenerator.Classical.CompareLessThan);
        ...
        ApplyXorInPlace(8, LittleEndian(b));
        CompareLessThan((a, b), c);

        let result = MResetZ(c);
...
namespace Microsoft.Quantum.OracleGenerator.Classical {
    ...
    internal function CompareLessThan(a : Int, b : Int) : Bool {
        return a < b;
    }
}
