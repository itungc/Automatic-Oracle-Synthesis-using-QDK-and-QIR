...
class qir_context
{
  public:
    qir_context(llvm::Module& module)
    {
        ...
        {
            if (*dest = llvm::StructType::getTypeByName(ctx, name); !dest)
            {
        ...
        // Added for the uncomputations
        h = module.getOrInsertFunction("__quantum__qis__h__body", llvm::Type::getVoidTy(ctx), qubitPtrTy);
        measure = module.getOrInsertFunction(
            "__quantum__qis__m__body", llvm::Type::getVoidTy(ctx), qubitPtrTy); //Not sure about the return type
        cz = module.getOrInsertFunction("__quantum__qis__cz__body", llvm::Type::getVoidTy(ctx), qubitPtrTy, qubitPtrTy);
    }
  ...
  private:
    ...
    llvm::FunctionCallee x, cnot, ccnot, allocate_array, array_update_alias_count, release_array, get_element, h, measure, cz;
};
    ...
    llvm::Function& function)
{
    ...
    const auto translate_and_gate = [&](abstract_xag_network::node n, uint32_t index, bool compute)
    {
        ...
        if (compute == 1) //compute case
        {
            builder.CreateCall(qir.CCNOT(), {node_to_value[diff[0].front()], node_to_value[diff[1].front()], temporary});
        }

        if (compute == 0) //uncompute case
        {
            // Use the applyAnd strucutre to reduce the uncomputation T-count
            builder.CreateCall(qir.H(), {node_to_value[diff[0].front()]});
            builder.CreateCall(qir.Measure(), {node_to_value[diff[0].front()]}); 
            builder.CreateCall(qir.CZ(), {node_to_value[diff[0].front()], node_to_value[diff[1].front()]});
        }
